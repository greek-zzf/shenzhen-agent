...
export async function upsertOptIn(params: {
  ...
  const sentAt =
    params.optedIn && arrivalChanged ? null : existing.sentAt;
...
/**
 * Future T+12h hook: rows opted in, not yet sent, now in [start+12h, start+24h).
 * Not scheduled — this repo has no wrangler cron / scripts job. Hotel rows are
 * never persisted (API rejects stayType=hotel).
 */
export async function listDueNudgeReminders(
  now = new Date()
): Promise<AccommodationReminder[]> {
  const rows = await db()
    .select()
